function calc(dt) {
    if (tmp.pass > 0) {
        ...
        if (player.feature >= 13) {
            if (!player.hadron.starter_upgs.includes(4)) {
                if (OA) {
                    ...
                    var m = mine_time.add((s ? tmp.super_mining_speed : tmp.mining_speed).div(FPS))
        ...
        if (player.feature >= 15) {
            ...
            if (forge.queue != "") {
                forge.time = player.hadron.starter_upgs.includes(3) ? EINF : forge.time.add(tmp.forge_speed.div(FPS))
        ...
        if (player.feature >= 16) {
            ...
            if (player.hadron.starter_upgs.includes(3)) {
                ...
                for (let i = 0; i < PARTICLE_ACCELERATOR.length; i++) {
                    ...
                    p.percent[i] = pp.max(pp.add(b4?dt*10:dt).min(PA.percent(CURRENCIES[PA.curr].amount))).max(0).min(1)
                }
            } else {
                if (a > -1) {
                    if (p.percent[a].lt(1)) {
