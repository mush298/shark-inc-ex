...
function updateCultivationHTML() {
    ...
    if (!hu4) for (let x = 0; x < 8; x++) {
        ...
        var h = `${toColoredText(lang[o.name])} ${toColoredText('×'+o.value.format(0))}<br>${toColoredText(o.health.format(2)+txt.heart,s?'orangered':'red')}`
    ...
    el("mining-damage").innerHTML = tmp.mining_damage.format(2)
    ...
    ORE_KEYS.forEach(x => {
        ...
        if (amt.gt(0)) r += `<div class='ore-list'>${c.costName+" ×"+amt.format(0)+(c.passive>0?" "+amt.formatGain(tmp.currency_gain[x].div(100)):"")}</div>`
    })
...
function getMiningDamage() {
    ...
    return x.pow(getPAEffect(2)).div(25)
}
