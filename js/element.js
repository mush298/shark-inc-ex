...
function updateHTML() {
    ...
    el('fish-amount').innerHTML = f.format(2) + (ff.length > 0 ? " " + ff.join("") : "")
    el('fish-gain').textContent = tmp.currency_gain.fish.gt(0) ? CURRENCIES.fish.amount.formatGain(tmp.currency_gain.fish.div(100)) : ""
...
function updateTopCurrenciesHTML() {
    for (let [i,x] of Object.entries(TOP_CURR)) {
        ...
        el(`curr-top-${i}-amt2`).textContent = c.amount.format(0) + ((c.passive??1)>0?" "+c.amount.formatGain(tmp.currency_gain[x.curr].mul(c.passive).div(100)):"")
